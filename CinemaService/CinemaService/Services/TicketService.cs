...
namespace CinemaService.Services
{
    public class TicketService : ITicketService
    {
        ...
        public void Create(TicketRequest ticketRequest)
        {
            ...
            int occupiedSeats = ticket.Projection.Theater.Seats.Where(x => x.Free == false).Count();

            int occupiedSeatsPurchase = occupiedSeats + ticketRequest.NumberOfTickets;

            if(occupiedSeatsPurchase <= ticket.Projection.Theater.Seats.Count())
            {
